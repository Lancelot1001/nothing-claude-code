...
Exposed 提供两种查询风格：DSL 用于直接类 SQL 表达式，DAO 用于实体生命周期管理。HikariCP 通过 `HikariConfig` 管理可重用数据库连接的连接池。Flyway 在启动时运行版本化 SQL 迁移脚本以保持架构同步。所有数据库操作都在 `newSuspendedTransaction` 块内运行，以实现协程安全和原子性。仓储模式将 Exposed 查询包装在接口后面，使业务逻辑与数据层解耦，测试可以使用内存 H2 数据库。
