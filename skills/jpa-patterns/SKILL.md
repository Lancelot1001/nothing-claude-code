...
- 使用 Flyway 或 Liquibase；切勿依赖 Hibernate 自动 DDL 在生产环境中
- 保持迁移幂等且增量；避免无计划地删除列

## 测试数据访问

- 优先使用 `@DataJpaTest` 和 Testcontainers 来镜像生产环境
- 使用日志断言 SQL 效率：设置 `logging.level.org.hibernate.SQL=DEBUG` 和 `logging.level.org.hibernate.orm.jdbc.bind=TRACE` 以查看参数值

**记住**：保持实体精简、查询有意、事务简短。使用获取策略和投影防止 N+1，并为读写路径建立索引。
