...
| 包装器 | 使用场景 |
|---------|----------|
| `@State` | 视图本地的值类型（开关、表单字段、sheet 显示） |
| `@Binding` | 到父级 `@State` 的双向引用 |
| `@Observable` class + `@State` | 带多个属性的自有模型 |
...
使用 `@Observable`（而非 `ObservableObject`）— 它追踪属性级别的变化，因此 SwiftUI 仅重新渲染读取了已变化属性的视图：
...
将视图分解为小型、专注的结构。当状态变化时，仅重新渲染读取该状态的子视图：
...
使用带有 `NavigationPath` 的 `NavigationStack` 实现程序化、类型安全的路由：
...
对于 body 昂贵的视图，实现 `Equatable` 以跳过不必要的重新渲染：
...
- 将异步工作直接放在 `body` 或 `init` 中 — 使用 `.task {}` 或显式 load 方法
- 在不拥有数据的子视图内创建 `@State` ViewModel — 从父级传递
- 使用 `AnyView` 类型擦除 — 优先使用 `@ViewBuilder` 或 `Group` 实现条件视图
- 在向 actors 传递数据时忽略 `Sendable` 要求

## 参考资源

参见skill：`swift-actor-persistence` 了解基于 actor 的持久化模式。
参见skill：`swift-protocol-di-testing` 了解基于协议的 DI 和使用 Swift Testing 进行测试。
