...
适用于 Android 和 KMP 项目的整洁架构模式。涵盖模块边界、依赖反转、UseCase/Repository 模式，以及使用 Room、SQLDelight 和 Ktor 进行数据层设计。
...
- 向 UI 层暴露数据库实体或 DTO——始终映射到 domain 模型
- 将业务逻辑放在 ViewModels 中——提取到 UseCase
- 使用 `GlobalScope` 或非结构化协程——使用 `viewModelScope` 或结构化并发
- 胖 repository 实现——拆分为专注的 DataSource
- 循环模块依赖——如果 A 依赖 B，B 不能依赖 A

## 参考

参见skill：`compose-multiplatform-patterns` 用于 UI 模式。
参见skill：`kotlin-coroutines-flows` 用于异步模式。
