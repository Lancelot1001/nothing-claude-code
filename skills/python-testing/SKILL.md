...
assert result is True  # 正好是 True
assert result is False  # 正好是 False
assert result is None  # 正好是 None
...
**记住**：测试也是代码。保持它们干净、可读和可维护。好的测试捕获缺陷；伟大的测试防止缺陷。
