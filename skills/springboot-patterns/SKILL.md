...
仅在以下情况下使用转发头：
1. 您的应用位于可信的反向代理（nginx、AWS ALB 等）后面
2. 您已将 `ForwardedHeaderFilter` 注册为 bean
3. 您已在 application properties 中配置了 `server.forward-headers-strategy=NATIVE` 或 `FRAMEWORK`
4. 您的代理配置为覆盖（而非追加到）`X-Forwarded-For` 头
...
使用 Spring 的 `@Scheduled` 或与队列集成（例如 Kafka、SQS、RabbitMQ）。保持处理器幂等且可观测。

## 可观测性

- 通过 Logback encoder 进行结构化日志记录（JSON）
...
- 适当通过 `@NonNull` 和 `Optional` 强制空安全

**记住**：保持 controllers 薄、services 专注、repositories 简单，错误集中处理。优化可维护性和可测试性。
