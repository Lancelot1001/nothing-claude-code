...
VideoDB 捕获会话的代码级细节。关于workflow process指南，请参阅 [capture.md](capture.md)。
...
使用 [scripts/ws_listener.py](../scripts/ws_listener.py) 连接并转储事件到 `${VIDEODB_EVENTS_DIR:-$HOME/.local/state/videodb}/videodb_events.jsonl`。
...
> 最新详情请参阅 [VideoDB 实时上下文文档](https://docs.videodb.io/pages/ingest/capture-sdks/realtime-context.md)。
...
| 方法 | 返回值 | 描述 |
|--------|---------|-------------|
| `conn.create_capture_session(end_user_id, collection_id, ws_connection_id, metadata)` | `CaptureSession` | 创建新捕获会话 |
...
> **注意：** `end_user_id` 是必需的，用于标识发起捕获的用户。对于测试或演示目的，任何唯一字符串标识符都可以（例如 `"demo-user"`、`"test-123"`）。
...
| `await client.start_capture_session(capture_session_id, channels, primary_video_channel_id)` | `None` | 开始流式传输选定通道 |
