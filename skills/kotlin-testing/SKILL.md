...
3. **模拟依赖**——使用 MockK 隔离被测试的单元
4. **运行测试（RED）**——验证测试以预期错误失败
5. **实现代码（GREEN）**——编写最少的代码使测试通过
6. **重构**——在保持测试通过的同时改进实现
7. **检查覆盖率**——运行 `./gradlew koverHtmlReport` 并验证 80%+ 覆盖率

## 示例

以下部分包含每个测试模式的详细可运行示例：

### 快速参考

- **Kotest specs**——StringSpec、FunSpec、BehaviorSpec、DescribeSpec 示例，见 [Kotest Spec 风格](#kotest-spec-styles)
- **Mocking**——MockK 设置、协程模拟、参数捕获，见 [MockK](#mockk)
- **TDD 演练**——使用 EmailValidator 的完整 RED/GREEN/REFACTOR 循环，见 [Kotlin TDD workflow](#tdd-workflow-for-kotlin)
- **覆盖率**——Kover 配置和命令，见 [Kover 覆盖率](#kover-coverage)
- **Ktor 测试**——testApplication 设置，见 [Ktor testApplication 测试](#ktor-testapplication-testing)
