...
PostgreSQL 最佳实践快速参考。获取详细指导，使用 `database-reviewer` agent。
...
- Agent：`database-reviewer`——完整数据库审查 workflow process
- Skill：`clickhouse-io`——ClickHouse 分析模式
- Skill：`backend-patterns`——API 和后端模式

---

*基于 Supabase Agent Skills（Credit: Supabase 团队）（MIT License）*
